...
import datetime
import requests  # For making HTTP requests to the anti-spoofing API
...
def load_embeddings(filename="embeddings_o2640.pkl"):
...
def check_liveness(face_image):
    """Send face image to the anti-spoofing API for liveness check."""
    try:
        ...
        url = "http://localhost:5000/compare"  # Replace with your API URL

        files = {'reference_image': ('face.jpg', image_bytes, 'image/jpeg')}
        ...
        if response.status_code == 200:
            ...
            return result.get("antispoofingproperty", False)
        else:
...
def process_frame(frame, embeddings, app, similarity_threshold=0.5):
    """Process a single frame for face detection, liveness check, and recognition."""
    try:
        rgb_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        resized_frame = cv2.resize(frame, (640, 360))
        faces = app.get(resized_frame)
        is_live = check_liveness(rgb_frame)

        for face in faces:
            if not hasattr(face, 'embedding'):
                continue

            # Extract face region for liveness detection
            ...
            cv2.rectangle(frame, (bbox[0], bbox[1]), (bbox[2], bbox[3]),
                          (0, 255, 0) if label not in ["Unknown", "Liveness Failed"] else (0, 0, 255), 2)
            cv2.putText(frame, label, (bbox[0], bbox[1] - 10),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0) if label not in ["Unknown", "Liveness Failed"] else (0, 0, 255), 2)
    except Exception as e:
